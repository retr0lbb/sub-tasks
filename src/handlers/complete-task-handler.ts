...
export async function toggleTaskCompletion({taskId, completion}: CompleteTaskParams){
    try {
        ...
        if(task.isCompleted === completion){
            throw new ClientError(`the task is already ${completion? "completed": "uncompleted"}`)
        }
