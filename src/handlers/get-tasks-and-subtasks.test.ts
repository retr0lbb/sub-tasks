...
import { getTasksAndSubtasks } from "./get-tasks-and-subtasks";
...
describe("Should get tasks and subtask", () => {
    ...
    it("should paginate correctly", async() => {
        ...
        const {tasks: taskList} = await getTasksAndSubtasks({numbersPerpage: numberOfRegistersPerPage, page: 0})
    ...
    it("shold return corret number of registries", async() => {
        vi.mocked(prisma.tasks.findMany).mockResolvedValue(Array.from({length: 50}, (_, index) => {
            return {
                createdAt: new Date(),
                description: "",
                isCompleted: false,
        ...
        const {tasks, numberOfEntries} = await getTasksAndSubtasks({numbersPerpage: 10, page: 1})
    ...
    it("should throw error if access a out of bounds page", async() => {
        vi.mocked(prisma.tasks.findMany).mockResolvedValue(Array.from({length: 10}, (_, index) => {
            return {
                createdAt: new Date(),
                description: "",
                isCompleted: false,
        ...
        await expect(getTasksAndSubtasks({page: 2, numbersPerpage: 10})).rejects.toBeInstanceOf(ClientError)
    })
